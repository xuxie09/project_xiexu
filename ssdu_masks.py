...
class ssdu_mask():
    ...
    def Gaussian_selection(self, input_data, input_mask, std_scale=4, num_iter=1):
        ...
        while count <= np.int64(np.ceil(np.sum(input_mask[:]) * self.rho)):

            indx = np.int64(np.round(np.random.normal(loc=center_kx, scale=(nrow - 1) / std_scale)))
            indy = np.int64(np.round(np.random.normal(loc=center_ky, scale=(ncol - 1) / std_scale)))
    ...
    def uniform_selection(self, input_data, input_mask, num_iter=1):
        ...
        ind = np.random.choice(np.arange(nrow * ncol),
                               size=np.int64(np.count_nonzero(pr) * self.rho), replace=False, p=pr / np.sum(pr))
